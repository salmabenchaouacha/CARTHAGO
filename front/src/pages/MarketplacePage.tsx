...
import { type ProductList } from '@/types';       // ← import
...
const MarketplacePage = () => {
  ...
  useEffect(() => {
    getProducts().then(data => {
      const cats = [...new Set(data.map(p => p.category__name))];
      setCategories(cats);
    });
  }, []);
  ...
  useEffect(() => {
    setLoading(true);
    getProducts({ category: category || undefined, region: region || undefined, q: search || undefined })
      ...
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          ...
          <>
            ...
            {products.length === 0 && <p className="text-center text-muted-foreground py-20">Aucun produit trouvé.</p>}
          </>
        )}
      </div>
    </div>
